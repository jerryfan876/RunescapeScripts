...
@ScriptManifest(author = "Jer",
        info = "Hitting Al-Kharid and pick up items",
        logo = "",
        name = "AlKharidNoPot",
        version = 1.0)
public class AlKharidNoPot extends Script {

    public static final int BANK_BOOTH_ID = 6943;
    public static final Area FIGHT_AREA = new Area(3282, 3167, 3303, 3177);
    public static final Area BANK_AREA = new Area(3269, 3163, 3271, 3170);
    public static final Position RESET_AGRO = new Position(3283, 3160, 0);
    ...
    private String[] LOOTLIST = new String[]{"Coins", "Chaos runes"};
    private GroundItem groundItem;

    private String needToKill = "Al-Kharid warrior";
    ...
    public State getState() {
        ...
        if (myPlayer().isUnderAttack() && areThereFood(STRINGFOOD) && !inventory.isFull()) {
            return State.FIGHTING;
            // lines that would run while in combat
        }

        if (FIGHT_AREA.contains(myPlayer().getPosition()) && !myPlayer().isUnderAttack() && areThereFood(STRINGFOOD) && !inventory.isFull()) {
            return State.FIND_TARGET;
            // lines that would run while finding a target
        }

        if (!areThereFood(STRINGFOOD) || inventory.isFull()) {
        ...
        switch (getState()) {
            ...
            case FIND_TARGET:
                ...
                if (!myPlayer().isUnderAttack()) {
                    sleep(random(1500, 2000)); //was (3400, 4000)
...
*/
            ...
            case FIGHTING:
                ...
                if (isPlayersAround()) {
                    log("+++FIGHTING we found a player");
                } else {
                    log("---FIGHTING we didnt find a player");
                }

                //check to see the the npc that we are fighting is the target that we want
                if (myPlayer().isUnderAttack() && getCombat().getFighting() != null) {
                    log("we are fighting: " + getCombat().getFighting().getName());
...
/*
                    for (String potionName : LISTOFPOTIONS) {
                        if (bank.contains(potionName + "(4)") && getBank().getAmount(potionName + "(4)") >= 5) {
                            ...
                            sleep(random(500, 1000));
                            getBank().withdraw((potionName + "(4)"), 3); //but now we needa check that there are enough space in the inventory to withdraw
    ...
    private List<NPC> allNPCs = getNpcs().getAll();
    private TreeMap<Integer , NPC> treeMapNpcs;

    SortedSet<Integer> keys = new TreeSet<>(treeMapNpcs.keySet());
    public Integer sortList () {
        for (int key: keys) {
            NPC value = treeMapNpcs.get(key);
        }
    }



    public List<NPC> sortNPC(){
        for (NPC npc : allNPCs){
            int distance = myPlayer().getPosition().distance(npc.getPosition());
        }
